...
uint32_t app_get_cur_version()
{
	uint32_t new_bin_ver = app_otas_get_curr_firmwave_version();
	return	new_bin_ver;
...
//            {
//                // ����̫��������ͨ��notify���أ�֪ͨclient����read��ʽ��ȡ
...
//write user data.
            if(rsp_hdr->rsp.write_data.base_address >= (app_otas_get_image_size()*2))
            {
                app_otas_save_data(rsp_hdr->rsp.write_data.base_address,p_data + (OTA_HDR_OPCODE_LEN+OTA_HDR_LENGTH_LEN)+sizeof(struct write_data_cmd),rsp_hdr->rsp.write_data.length);
                break;
            }
			...
			#ifdef OTA_CRC_CHECK			
            if((rsp_hdr->rsp.write_data.base_address !=(ota_addr_check + ota_addr_check_len)) &&(rsp_hdr->rsp.write_data.base_address !=ota_addr_check))
            {//for OTA write addr error  no req
                co_printf("rsp_hdr->rsp.write_data.base_address = %x\r\nota_addr_check=%x,\r\nlen = %d\r\nSUM=%x\r\n",rsp_hdr->rsp.write_data.base_address,ota_addr_check,len,rsp_hdr->rsp.write_data.base_address + len);
                os_free(req);
                ota_stop(OTA_ADDR_ERROR);
                return;
