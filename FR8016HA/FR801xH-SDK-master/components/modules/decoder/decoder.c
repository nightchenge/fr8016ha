...
uint8_t stop_flag = 0;
uint8_t decodeTASKState = DECODER_STATE_IDLE;//����decoder_play_next_frame_handler �е�״̬
 ...
 * @return	None.
 */
void test_speaker_from_flash(uint32_t playaddr)
{
    ...
    //flash_read(USER_FLASH_BASE_ADDR, sizeof(sbc_sotre_env), (uint8_t *)&sbc_sotre_env);//��ȡFlash�洢��audio��Ϣ
    sbc_sotre_env.start_base=playaddr;
	sbc_sotre_env.tot_data_len=2172;
	sbc_sotre_env.last_page_idx=1;
	sbc_sotre_env.last_offset=1148;
    co_printf("%x,%d,%d,%d\r\n",sbc_sotre_env.start_base,sbc_sotre_env.tot_data_len,sbc_sotre_env.last_page_idx,sbc_sotre_env.last_offset);
    if(sbc_sotre_env.start_base == 0xffffffff)//�쳣
    {
        memset((void *)&sbc_sotre_env,0x00,sizeof(sbc_sotre_env));
		Flash_data_state = false;//flashû����Ƶ����
        goto _Exit;
    }
	Flash_data_state = true;//flash������Ƶ����
    speaker_init();//speaker ��ʼ��

    sbc_buff = (uint8_t *)os_zalloc(4*1024);//����10kbuffer


    uint8_t *tmp_buf;

    flash_read(sbc_sotre_env.start_base, 512, sbc_buff);//��ȡ512���ֽڵ�����

    memset((void *)&speaker_env, 0, sizeof(speaker_env));
    tmp_buf = sbc_buff;
    speaker_env.sbc_frame_len = decoder_calc_adpcm_ms_frame_len(&tmp_buf);//��ȡsbc_frame_len
    ...
    if(speaker_env.sbc_data_tot_len == 0)
        speaker_env.sbc_data_tot_len = (4096 - 4096%speaker_env.sbc_frame_len)&(~0x1);
